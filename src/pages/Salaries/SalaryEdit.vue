<template>
  <div class="container mt-4">
    ...
    <form @submit.prevent="updateForm">
      ...
      <!-- Submit -->
      <button type="submit" class="btn btn-success" :disabled="isSubmitting">Update Salary</button>
    </form>
  </div>
</template>
...
onMounted(async () => {
  await fetchEmployeesAndItems()
  await fetchSalary()
})

async function fetchEmployeesAndItems() {
  try {
    ...
    console.error('Error loading employees or items', err)
  }
}

async function fetchSalary() {
  try {
    const res = await axios.get(baseUrl + `salaries/${salaryId}`)
    const salary = res.data?.data

    formData.value.employee_id = salary.employee_id
    formData.value.remarks = salary.remarks
    formData.value.status = salary.status
    formData.value.items = salary.items.map(item => ({
...
async function updateForm() {
  try {
    ...
    const res = await axios.put(baseUrl + `salaries/${salaryId}`, payload)
